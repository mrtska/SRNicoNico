 ...
 */
(function ($) {
    var style = $('<style/>').appendTo('head')[0].sheet, isObj = $.isPlainObject, isFunc = $.isFunction, isNum = $.isNumeric;
    function createKey() {
        var str = 'abcdefghijklmnopqrstuvwxyz0123456789', len = str.length, key = 'keyframes-', i = 0;
        for (; i < 10; i++) {
    ...
    $.fn.keyframes = function (keys, opt, callback) {
        return this.each(function () {
            var that = this, $this = $(that), data = that.keyframes, iteration, key, steps, set;
            opt = opt || {};
            if (!isObj(data)) {
                that.animated = false;
                that.paused = false;
